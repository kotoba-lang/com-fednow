(ns fednow.main
  "Kotodama WASM entrypoint for the Fednow clean-room actor (L4) — Clojure port.
  ...
  facts (`fednow.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fednow")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "fednow_fil"
    :fields [:filerId :form :period :status]                :required [:form :period]
    :coerce {}                                   :refs {}}
   {:entity "Entity"       :plural "entities"      :id-prefix "fednow_ent"
    :fields [:name :ein :jurisdiction]                     :required [:name :ein]
    :coerce {}                                   :refs {}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "fednow_dat"
    :fields [:name :agency :updatedAt]                     :required [:name :agency]
    :coerce {}                                   :refs {}}
   {:entity "Application"  :plural "applications"  :id-prefix "fednow_app"
    :fields [:applicantId :program :status]                :required [:program :status]
    :coerce {}                                   :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "fednow_pub"
    :fields [:recordType :jurisdiction :recordedAt]        :required [:recordType :jurisdiction]
    :coerce {}                                   :refs {}}
   {:entity "Notice"       :plural "notices"       :id-prefix "fednow_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"fednow.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fednow-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fednow-compat/ready)
